...
export const Placement = (props: { shown: boolean }) => {
  const { shown } = props;
...
const CodeView = () => {
  ...
  const displayPlacement = (lineNumber: number) => {
      ...
      lineProps={(lineNumber) => {
        return {
          style: highlightedLines.includes(lineNumber)
            ...
            : {},
          onClick: () => displayPlacement(lineNumber)
        };
      }}
  ...
  return (
    <>
      <Placement shown={showPlacement}></Placement>
      {sourceCode
        ? sourceCodeView
        : placeholder}
    </>);
